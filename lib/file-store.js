...
let writeFileAsync = utils.promisify(fs.writeFile)
let readFileAsync = utils.promisify(fs.readFile)
...
FileStore.prototype.save = async function (set) {
  await writeFileAsync(this.path, JSON.stringify({
    lastRun: set.lastRun,
...
FileStore.prototype.load = async function () {
  let json
  try {
    json = await readFileAsync(this.path, 'utf8')
  } catch (error) {
